#!/usr/bin/env python3
"""
ZAMANLANMIŞ MESAJ BOT - DEBUG MOD
"""
...
async def send_scheduled_message(schedule_item, messages_dict):
    """Zamanlanmış mesajı gönder - DEBUG"""
    try:
        ...
        # ⭐ DEBUG: Ne gönderdiğimizi görelim!
        print("\n" + "="*50)
        print("🔍 DEBUG - GÖNDERİLECEK MESAJ:")
        print("="*50)
        print(f"Orijinal mesaj (JSON'dan):")
        print(f'"{message_text}"')
        print(f"Uzunluk: {len(message_text)} karakter")
        print(f"Son 20 karakter: '{message_text[-20:]}'")
        print("="*50 + "\n")
        
        # KESİNLİKLE HİÇBİR ŞEY EKLEME
        final_message = message_text
        
        # ⭐ DEBUG: Telegram'a ne gönderdiğimizi kaydedelim
        with open("debug_log.txt", "a", encoding="utf-8") as f:
            f.write(f"\n[{datetime.now()}] Gönderilen mesaj:\n")
            f.write(final_message)
            f.write(f"\nUzunluk: {len(final_message)} karakter\n")
            f.write("-" * 40 + "\n")
